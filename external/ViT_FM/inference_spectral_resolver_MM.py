...
if __name__ == "__main__":
    ...
    if not ignore_diffusion:
        ...
        diffusion_tag = f"spectral_resolver_{sde_steps}_steps_{num_ensemble}_ensemble_{renormalize}_renorm_{guidance_strength}_guidance{additional_tag}"
    ...
    for folder in folders:
        ...
        for curr_macro in range(N_macro):
            ...
            if save_data:
                counter = 0
                if not os.path.exists(f"{path}/predictions_{diffusion_tag}_{config.which_data}_{inference_tag}{N_macro}_macro/predictions_{diffusion_tag}_MMcurr_{curr_macro}_{N_micro}_{config.which_data}_{inference_tag}"):
                    os.makedirs(f"{path}/predictions_{diffusion_tag}_{config.which_data}_{inference_tag}{N_macro}_macro/predictions_{diffusion_tag}_MMcurr_{curr_macro}_{N_micro}_{config.which_data}_{inference_tag}")
                folder_pred = f"{path}/predictions_{diffusion_tag}_{config.which_data}_{inference_tag}{N_macro}_macro/predictions_{diffusion_tag}_MMcurr_{curr_macro}_{N_micro}_{config.which_data}_{inference_tag}"
