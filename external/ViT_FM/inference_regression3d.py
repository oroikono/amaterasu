...
if __name__ == "__main__":
    ...
    if ("finetuned" in folder_) or (("scratch" in folder_ or "Scratch" in folder_) and "scratch_Base" not in folder_):
    ...
    for folder in folders:
        ...
        test_loader = get_loader(which_data = config.which_data,
                            ...
                            rel_time = False,
                            ood_tag = None,
                            curr_macro = n_macro) #For the micro_macro eval (spectral resolver)
        ...
        dtype = next(regression_model.parameters()).dtype  # model’s dtype
        ...
        with torch.no_grad():
            for step, batch in enumerate(tqdm_data):
                ...
                input_batch = input_batch.to(device=device, dtype=dtype)
                output_batch = output_batch.to(device=device, dtype=dtype)
                input_batch_copy = copy.deepcopy(input_batch).to(device=device, dtype=dtype)
                ...
                if is_time_reg:
                    for i, reg_step in enumerate(regression_scheme):
                        t = dt * reg_step
                        t_batch = t * torch.ones_like(t_batch).to(device=device)
                ...
                if save_data:
                    ...
                    for i in range(pred_copy.shape[0]):
                        np.save(f"{folder_pred}/sample_{cnt_pred}_{macro_tag}pred.npy", pred_copy[i])
                        np.save(f"{folder_pred}/sample_{cnt_pred}_{macro_tag}out.npy", output_copy[i])
                        np.save(f"{folder_pred}/sample_{cnt_pred}_{macro_tag}inp.npy",input_batch_copy.detach().cpu().numpy()[i])
                        cnt_pred+=1
                    del pred_copy
                ...
                output_pred_batch = output_pred_batch * std + mean
