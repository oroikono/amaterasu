...
def create_denoiser(
    ...
    param_dim: int = 0,
    param_embed_channels: int = 0,
):
    """Get the denoiser and sampler if required"""

    if param_embed_channels > 0 and (args.local_rank == 0 or args.local_rank == -1):
        print(
            "Warning: `param_embed_channels` is ignored when using FiLM parameter conditioning."
        )
    ...
    model = get_model(
        ...
        # Parameter conditioning is applied via FiLM in DStack and does not add input channels.
        out_channels=out_channels,
        spatial_resolution=spatial_resolution,
        time_cond=time_cond,
        device=device,
