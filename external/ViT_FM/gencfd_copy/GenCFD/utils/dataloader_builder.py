...
from GenCFD.dataloader.fluid_flows_3d import (
    ShearLayer3D,
    ShearLayerSpectral3D,
    TaylorGreen3D,
    Nozzle3D,
    ATM_MSC_3D_moist,
    ATM_MSC_3D_dry,
    ConditionalShearLayer3D,
    ConditionalShearLayerSpectral3D,
    ConditionalTaylorGreen3D,
    ConditionalNozzle3D,
    ConditionalATM_MSC_3D_moist
)
...
def get_dataset(
    ...
    if name == "ShearLayer2D":
        dataset = ShearLayer2D(metadata=metadata, training_samples=training_samples)
        time_cond = False

    elif name == "CloudShock2D":
        dataset = CloudShock2D(metadata=metadata, training_samples=training_samples)
        time_cond = False

    elif name == "RichtmyerMeshkov2D":
        dataset = RichtmyerMeshkov2D(metadata=metadata, training_samples=training_samples)
        time_cond = False

    elif name == "ShearLayer3D":
        dataset = ShearLayer3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ShearLayerSpectral3D":
        dataset = ShearLayerSpectral3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "TaylorGreen3D":
        dataset = TaylorGreen3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "Nozzle3D":
        dataset = Nozzle3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ATM_MSC_3D_moist":
        dataset = ATM_MSC_3D_moist(file_system=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ATM_MSC_3D_dry":
        dataset = ATM_MSC_3D_dry(file_system=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ConditionalShearLayer2D":
        dataset = ConditionalShearLayer2D(metadata=metadata, training_samples=training_samples)
        time_cond = False

    elif name == "ConditionalCloudShock2D":
        dataset = ConditionalCloudShock2D(metadata=metadata, training_samples=training_samples)
        time_cond = False

    elif name == "ConditionalShearLayer3D":
        dataset = ConditionalShearLayer3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ConditionalShearLayerSpectral3D":
        dataset = ConditionalShearLayerSpectral3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ConditionalTaylorGreen3D":
        dataset = ConditionalTaylorGreen3D(metadata=metadata, training_samples=training_samples)
        time_cond = True

    elif name == "ConditionalNozzle3D":
        dataset = ConditionalNozzle3D(metadata=metadata)
        time_cond = True
    elif name == "ConditionalATM_MSC_3D_moist":
        dataset = ConditionalATM_MSC_3D_moist(file_system=metadata, training_samples=training_samples)
        time_cond = True
...
def optional_parameters_collate(batch: list[dict]):
    """Collate function that keeps optional `parameters` as `None` when absent."""
    collated = {}
    for key in batch[0]:
        values = [item[key] for item in batch]
        if key == "parameters" and values[0] is None:
            collated[key] = None
        else:
...
def get_dataset_loader(
    ...
    if split:
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=optional_parameters_collate,
        )
