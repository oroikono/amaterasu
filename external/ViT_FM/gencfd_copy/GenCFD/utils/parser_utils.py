...
def add_base_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default=1,
        type=int,
        help="Number of mini-batches to accumulate before each optimizer update.",
    )
...
def add_data_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default=None,
        type=int,
        help="Optional number of training samples to load from the selected dataset",
    )
...
def add_trainer_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default=1.0,
        type=float,
        help="Maximum L2 norm for gradient clipping (set <=0 to disable).",
    )
...
def add_training_options(parser: ArgumentParser):
    ...
    group.add_argument(
        ...
        default=False,
        help=(
            "If set, freezes param_scale.weight and param_shift.weight during warmup "
            "and unfreezes them afterwards."
        ),
    )
    group.add_argument(
        "--warmup_steps",
        ...
        type=int,
        help=(
            "Override the number of warmup steps used by the trainer. If omitted, "
            "entrypoint defaults are used."
        ),
    )
    group.add_argument(
        "--min_lr",
        ...
        type=float,
        help=(
            "Override the minimum learning rate used by the scheduler. If omitted, "
            "entrypoint defaults are used."
        ),
    )
    
    group.add_argument(
        "--warmup_finetune_scratch_only",
        action="store_true",
        default=False,
        help=(
            "If set during fine-tuning, warmup updates only parameters that were "
            "initialized from scratch. All parameters are unfrozen automatically "
            "after warmup."
        ),
    )
    ...
    group.add_argument(
        ...
        default=False,
        help=(
            "If set during fine-tuning, only parameters initialized from scratch "
            "are trainable for the entire training run (including after warmup)."
        ),
    )
    group.add_argument(
        "--finetune_unfreeze_ustack3d_output_after_warmup",
        action="store_true",
        default=False,
        help=(
            "Only for 3D fine-tuning with --finetune_scratch_only_entire_training: "
            "after warmup, additionally unfreezes UStack3D res_skip_layer and the "
            "last output conv layer while all other pretrained parameters remain frozen."
        ),
    )

    group.add_argument(
        "--debug_track_param_name",
        default=None,
        type=str,
        help=(
            "Optional parameter name to print during training in order to verify "
            "that it is being updated (e.g. `UStack.attention_blocks.0.1...`)."
        ),
    )
    group.add_argument(
        "--debug_track_param_log_every",
        default=100,
        type=int,
        help="Log tracked-parameter update information every N optimizer steps.",
    )
