...
class DenoisingModel(BaseModel):
    ...
    def loss_fn(self, batch: dict, mutables: Optional[dict] = None):
        ...
        parameters = batch.get("parameters") if "parameters" in batch else None 
        ...
        if time is not None:
            ...
            denoised = self.denoiser.forward(x=noised, y=y, sigma=sigma, time=time, parameters=parameters)
        else:
            #denoised = self.denoiser.forward(x=noised, y=y, sigma=sigma)
            denoised = self.denoiser.forward(x=noised, y=y, sigma=sigma, parameters=parameters)
    ...
    def eval_fn(self, batch: dict) -> dict:
        ...
        if parameters is not None:
            param_inputs = parameters[rand_idx_set]
        ...
        if time is not None:
            denoised = torch.stack(
                [
                    denoise_fn(
                        ...
                        time=time_inputs[i],
                        parameters=None if parameters is None else param_inputs[i],
                    )
            ...
            denoised = torch.stack(
                [
                    denoise_fn(x=noised[i], y=y[i], sigma=sigma[i], parameters=None if parameters is None else param_inputs[i])
                    for i in range(self.num_eval_noise_levels)
                ]
            )
    ...
    def inference_fn(
        ...
        if lead_time == False:
            ...
            def _denoise(
                ...
                return denoiser.forward(x=x, sigma=sigma, y=y, parameters=parameters)
            ...
            def _denoise(
                ...
                return denoiser.forward(x=x, sigma=sigma, y=y, time=time, parameters=parameters)
        else:
            raise ValueError(
