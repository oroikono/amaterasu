...
class PreconditionedDenoiser(UNet):
    ...
    def __init__(
        ...
        if self.param_dim > 0:
            ...
            nn.init.normal_(self.DStack.param_scale.weight, mean=0.0, std=1e-3)
            nn.init.normal_(self.DStack.param_shift.weight, mean=0.0, std=1e-3)
    ...
    def forward(
        ...
        f_x = super().forward(
            inputs, sigma=c_noise, time=time, down_only=down_only, parameters=parameters
        )
