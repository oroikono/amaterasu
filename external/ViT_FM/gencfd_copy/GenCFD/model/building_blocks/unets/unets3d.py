...
class UNet3D(nn.Module):
    ...
    def forward(
        ...
        skips = self.DStack(x, emb, parameters=parameters)
...
class PreconditionedDenoiser3D(UNet3D, nn.Module):
    ...
    def __init__(
        ...
        self.DStack.param_dim = self.param_dim
        if self.param_dim > 0:
            #self.param_embed = nn.Linear(self.param_dim, int(torch.tensor(self.spatial_resolution).prod().item()), device=self.device, dtype=self.dtype)
            self.DStack.param_scale = nn.Linear(
                self.param_dim,
                self.input_proj_channels,
                bias=False,
                device=self.device,
                dtype=self.dtype,
            )
            self.DStack.param_shift = nn.Linear(
                self.param_dim,
                self.input_proj_channels,
                bias=False,
                device=self.device,
            ...
            nn.init.constant_(self.DStack.param_scale.weight, 0.0)#(self.DStack.param_scale.weight, mean=0.0, std=1e-3)
            nn.init.constant_(self.DStack.param_shift.weight, 0.0)#( mean=0.0, std=1e-3)
    ...
    def forward(
        ...
        f_x = super().forward(
            inputs, sigma=c_noise, time=time, cond=cond, parameters=parameters
        )
