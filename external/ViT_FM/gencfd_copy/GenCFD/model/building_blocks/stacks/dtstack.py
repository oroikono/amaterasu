...
class DStack(nn.Module):
    ...
    def __init__(
        ...
        if self.param_dim > 0:
            self.param_scale = nn.Linear(
                self.param_dim,
                self.num_input_proj_channels,
                bias=False,
                device=self.device,
                dtype=self.dtype,
            )
            self.param_shift = nn.Linear(
                self.param_dim,
                self.num_input_proj_channels,
                bias=False,
                device=self.device,
            ...
            nn.init.normal_(self.param_scale.weight, mean=0.0, std=1e-3)
            nn.init.normal_(self.param_shift.weight, mean=0.0, std=1e-3)
    ...
    def forward(self, x: Tensor, emb: Tensor, parameters: Tensor | None = None) -> list[Tensor]:
        ...
        if parameters is not None:
            if self.param_scale is None or self.param_shift is None:
                raise ValueError(
                    "Received `parameters` but FiLM parameter layers are not initialized."
                )
            if parameters.ndim != 2 or parameters.shape[0] != x.shape[0]:
                raise ValueError(
                    "`parameters` must be of shape (batch, L) with matching batch size."
                )

            params = parameters.to(device=h.device, dtype=h.dtype)
            scale = self.param_scale(params)
            shift = self.param_shift(params)
            expand_shape = [h.shape[0], h.shape[1]] + [1] * (h.ndim - 2)
            h = (1 + scale.view(*expand_shape)) * h + shift.view(*expand_shape)
