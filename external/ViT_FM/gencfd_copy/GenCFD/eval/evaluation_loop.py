...
from GenCFD.utils.eval_utils import summarize_metric_results, spectrum_from_physical_Linf_per_sample
...
def run(
    *,
    ...
    save_gen_samples_in_file: bool = False,
    ...
    #print(dataloader.dataset.mean_training_output)
    #time.sleep(100)
    ...
    if compute_metrics:
        ...
        nc_ds = None
        write_offset = 0  # member index
        if save_gen_samples_in_file and (local_rank == 0 or local_rank == -1):
            tag = save_dir.strip().split("/")[-2]
            nc_name = f"gencfd_data_gen_{tag}.nc"
            path = "/cluster/work/math/braonic/TrainedModels/GenCFD/TG3D_rebuttal_generated"
            nc_path = os.path.join(path, nc_name)
            nc_ds = nc.Dataset(nc_path, mode="w", format="NETCDF4")

            # Dimensions
            nc_ds.createDimension("member", None)  # unlimited
            nc_ds.createDimension("x", 64)
            nc_ds.createDimension("y", 64)
            nc_ds.createDimension("z", 64)

            # Chunking helps performance for streaming writes
            chunks = (1, 64, 64, 64)

            # Generated variables
            var_u = nc_ds.createVariable("u", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)
            var_v = nc_ds.createVariable("v", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)
            var_w = nc_ds.createVariable("w", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)

            # Target variables (optional but you asked to save u as well)
            var_u_true = nc_ds.createVariable("u_true", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)
            var_v_true = nc_ds.createVariable("v_true", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)
            var_w_true = nc_ds.createVariable("w_true", "f4", ("member", "x", "y", "z"), zlib=True, chunksizes=chunks)
        ...
        if dataset.mean_training_output is not None:
            mean_output = torch.tensor(dataset.mean_training_output).to(device, non_blocking=True).detach()
            std_output =  torch.tensor(dataset.std_training_output).to(device, non_blocking=True).detach()
        else:
            mean_output = torch.tensor([0]).to(device, non_blocking=True).detach()
            std_output =  torch.tensor([1]).to(device, non_blocking=True).detach()
        ...
        for i in range(n_iter):
            ...
            #gen_samples = u0[:, -3:].detach()

            if i == 0:
                U_gen = gen_samples.cpu().numpy()
                U = u.cpu().numpy()
            else:
                U_gen = np.concatenate((U_gen, gen_samples.cpu().numpy()))
                U = np.concatenate((U, u.cpu().numpy()))

            #print(gen_samples.shape, mean_output.shape, u.shape, gen_samples.shape)
            
            #print(std_output, mean_output)
            #time.sleep(100)
            gen_samples = gen_samples * std_output.view(1, -1, 1, 1, 1) + mean_output.view(1, -1, 1, 1, 1)
            u = u * std_output.view(1, -1, 1, 1, 1) + mean_output.view(1, -1, 1, 1, 1)

            print(torch.max(gen_samples[0,0]), torch.max(u[0,0]))
            #print(gen_samples.shape, u.shape)
            # mask = dataset.get_mask().unsqueeze(0).unsqueeze(0).cuda()
            # u = u * mask
            stats_recorder.update_step(gen_samples, u)


            if save_gen_samples_in_file and nc_ds is not None:
                # gen_samples, u are (B, 3, 64, 64, 64)
                # we want (B, 64, 64, 64) per channel
                b = gen_samples.shape[0]
                sl = slice(write_offset, write_offset + b)

                # move to CPU + float32
                gs = copy.deepcopy(gen_samples).detach().cpu().numpy()
                uu = copy.deepcopy(u).detach().cpu().numpy()

                # write generated channels
                var_u[sl, :, :, :] = gs[:, 0, :, :, :]
                var_v[sl, :, :, :] = gs[:, 1, :, :, :]
                var_w[sl, :, :, :] = gs[:, 2, :, :, :]

                # write target channels
                var_u_true[sl, :, :, :] = uu[:, 0, :, :, :]
                var_v_true[sl, :, :, :] = uu[:, 1, :, :, :]
                var_w_true[sl, :, :, :] = uu[:, 2, :, :, :]

                write_offset += b
                nc_ds.sync()  # optional: makes data appear on disk incrementally
        ...
        if (world_size > 1 and local_rank == 0) or world_size == 1:
            ...
            _, spectrum_gen, counts = spectrum_from_physical_Linf_per_sample(U_gen)
            _, spectrum_gt, counts_gt= spectrum_from_physical_Linf_per_sample(U)
