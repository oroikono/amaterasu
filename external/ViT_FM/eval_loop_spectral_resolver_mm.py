...
def run(
    *,
    ...
    for curr_macro in range(N_macro):
        folder_samples = f"{prefix}{curr_macro}_{N_micro}{sufix}"
        save_dir = folder_samples
        ...
        if compute_metrics:
            ...
            metrics_dict = summarize_metric_results(stats_recorder, save_dir, err = errs, err_rel = errs_rel)
            print(metrics_dict)
            ...
            deterministic_rel = []
            '''
            mean_rel.append(metrics_dict["mean"]["relative"])
            mean_absolute.append(metrics_dict["mean"]["absolute"])
            std_rel.append(metrics_dict["std"]["relative"])
            w = []
            for i in range(C):
                w.append(metrics_dict["wasserstein_distance"][f"wass_{i}"])
            wass_dist.append(w)
            deterministic_rel.append(metrics_dict["determinsitic_relative_l1"])
    ...
    mean_rel = np.median(np.array(mean_rel), axis = 0)
    mean_absolute = np.median(np.array(mean_absolute), axis = 0)
    std_rel = np.median(np.array(std_rel) , axis = 0)
    wass_dist = np.median(np.array(wass_dist), axis = 0)
    deterministic_rel = np.median(np.array(deterministic_rel) ,axis = 0)

    wass_distance = dict()
    for i in range(C):
        wass_distance[f"wass_{i}"] = wass_dist[i]

    # Construct the metrics dictionary
    metrics_dict_mm = {
        "mean": {
            "relative": list(mean_rel),
            "absolute": list(mean_absolute),
        },
        "std": {
            "relative": list(std_rel),
        },
        "wasserstein_distance": wass_distance,
    }

    metrics_dict_mm["deterministic_rel"] = list(deterministic_rel)

    save_file = f"{folder_save_mm}/metrics_results_mm_{N_macro}_{monte_carlo_samples}.json"
    with open(save_file, "w") as f:
        json.dump(metrics_dict_mm, f, indent=4)
    
    print(mean_rel.shape)
    print(std_rel.shape)
    print(wass_dist.shape)
    print(deterministic_rel.shape)
...
folder_save_mm = "/cluster/work/math/braonic/TrainedModels/OOD_Generalization/pdegym_plus/PDEGYM_PLUS_10ep_ViTB_regression/finetuned/ns_shear_gencfd/pdegym_plus_80k_FT_ns_shear_gencfd_non_native_80000/predictions_pure_regression_True_renorm__ns_shear_gencfd_mm_10_macro"
prefix = "/cluster/work/math/braonic/TrainedModels/OOD_Generalization/pdegym_plus/PDEGYM_PLUS_10ep_ViTB_regression/finetuned/ns_shear_gencfd/pdegym_plus_80k_FT_ns_shear_gencfd_non_native_80000/predictions_pure_regression_True_renorm__ns_shear_gencfd_mm_10_macro/predictions_pure_regression_True_renorm__MMcurr_"
sufix = "_ns_shear_gencfd_mm_"
mask_group = [2]
