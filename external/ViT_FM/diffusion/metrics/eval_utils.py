...
from diffusion.metrics.probabilistic_forecast import relative_L2_norm, absolute_L2_norm
from diffusion.metrics.wasserstein import compute_average_wasserstein
...
def summarize_metric_results(
    ...
    if err is not None:
        metrics_dict["determinsitic_l1"] = list(err)
    if err_rel is not None:
        metrics_dict["determinsitic_relative_l1"] = list(err_rel)
